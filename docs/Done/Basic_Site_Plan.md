...
todos:
  ...
  - id: robots-txt
    ...
    status: completed
  - id: robots-txt-fix
    content: Remove robots.txt from main branch so production site is crawlable
    status: completed
isProject: false
---
...
- Hosted on **Firebase Hosting** with two sites (production + test)
- Deployed via GitHub Actions on push to `main` (production) or `test` (test site)
- Domain registered via Squarespace, DNS managed via Cloudflare (free tier)
- Firebase handles HTTPS automatically and serves `404.html` by convention
...
## Step 2: Generate a Firebase Service Account Key (Manual) ✅

For GitHub Actions to deploy, you need a service account key:

1. In Firebase Console, go to **Project Settings > Service accounts**
2. Click **Generate new private key** — a JSON file will download
3. Add the **entire JSON contents** as a GitHub repository secret named `FIREBASE_SERVICE_ACCOUNT`
...
**Important:** For multi-site hosting with deploy targets, `hosting` must be an **array** with a `target` field on each entry.
...
**Authentication:** The workflows use a **service account key** stored as the `FIREBASE_SERVICE_ACCOUNT` GitHub secret (see Step 2).
...
A `robots.txt` file exists **only on the `test` branch** to block search engine indexing of the test site:
...
This file was removed from `main` so the production site is fully crawlable.
...
- ~~The `.htaccess` file can be deleted~~ — Done
- ~~Remove the old InfinityFree secrets from GitHub~~ — `INFINITY_FTP_HOST`, `INFINITY_FTP_USERNAME`, `INFINITY_FTP_PASSWORD` should be removed if still present
